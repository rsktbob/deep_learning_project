...
from data_pipeline import download_yahoo_data
...
# %% Part 5. Taiwan 50 ETF baseline

baseline_end = (pd.to_datetime(trade["date"].max()) + pd.Timedelta(days=1)).strftime(
    "%Y-%m-%d"
)
df_tw50 = download_yahoo_data(
    ["0050.TW"],
    trade["date"].min(),
    baseline_end,
)[["date", "close"]]
fst_day = df_tw50["close"].iloc[0]
tw50 = pd.merge(
    df_tw50["date"],
    df_tw50["close"].div(fst_day).mul(1000000),
    how="outer",
    left_index=True,
    right_index=True,
).set_index("date")
